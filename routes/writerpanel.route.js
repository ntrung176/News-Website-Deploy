...
router.get("/", async function (req, res) {
  if (req.isAuthenticated() && req.user.Permission === 1) {
    ...
    const post_ChuaDuyet = await postModel.singleByUserIDStatus(
      req.user.UserID,
      0
    );
    const post_TuChoi = await postModel.singleByUserIDStatus(
      req.user.UserID,
      1
    );
    const post_ChoXuatBan = await postModel.singleByUserIDStatus(
      req.user.UserID,
      2
    );
    const post_XuatBan = await postModel.singleByUserIDStatus(
      req.user.UserID,
      3
    );
    for (var i = 0; i < post_by_UID.length; i++) {
      post_by_UID[i].Time = moment(
        post_by_UID[i].TimePost,
        "YYYY-MM-DD hh:mm:ss"
      ).fromNow();
