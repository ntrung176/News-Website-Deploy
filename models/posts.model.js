...
module.exports = {
  ...
  del2: async function (id) {
    try {
      await db.load(`DELETE FROM comment WHERE PostID = ${id}`);
      await db.load(
        `DELETE FROM categorymanager WHERE CID IN (SELECT CID FROM posts WHERE PostID = ${id})`
      );
      return db.load(`DELETE FROM ${TBL_POSTS} WHERE PostID = ${id}`);
    } catch (error) {
      throw error;
    }
  },
